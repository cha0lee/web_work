...
public class FriendUpdateFormServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest request, 
		...
		//2. MemberDao 객체를 이용해서 해당 번호의 친구정보를 얻어온다.
		FriendDto dto=FriendDao.getInstance().getData(num);
		//3. 수정할 친구의 정보를 form 에 출력해서 응답한다.
		//응답 인코딩 설정
		...
		pw.println("<title>친구정보 수정폼</title>");
		pw.println("</head>");
		pw.println("<body>");
		/*
		...
		pw.println("<input type='text' name='regdate' value='"+dto.getRegdate()+"' />");
